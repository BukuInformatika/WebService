\documentclass{wileySix}
\usepackage{w-bookps}


\usepackage{graphicx}
\usepackage{enumitem}

\setcounter{secnumdepth}{3}

\setcounter{tocdepth}{2}

\begin{document}

\booktitle{Web Service}
\subtitle{Semua Tentang Komunikasi antar Aplikasi Berbasis Protokol internet}

\author{Rolly Maulana Awangga}

\halftitlepage
\titlepage

\offprintinfo{Web Service, pre-release}{Rolly Maulana Awangga}


\begin{copyrightpage}{2018}
Web Service / Rolly Maulana Awangga
\end{copyrightpage}


\dedication{For my family}

\contentsinbrief %optional
\tableofcontents
\listoffigures %optional
\listoftables  %optional

%%%%%%%%%
%%Content 
%%%%%%%%%

\part[Pengenalan Web Service]
{Pengenalan\\ Web Service}

%\chapter[Contoh]
%{Contoh\\ Latex}
%\input{section/1.tex}

%\chapter[3FlaskGet]
%{Contoh\\ 3FlaskGet}
%\input{section/3FlaskGet.tex}

\chapter[3FlaskPOST]
{Contoh\\ 3FlaskPOST}
\input{section/3FlaskPOST.tex}


\chapter[3FlaskPOST]
{Contoh\\ 3FlaskPOST}
\input{section/3FlaskPOST.tex}

%\chapter[RESTful]
%{Definisi\\ RESTful}
%\input{section/1RESTful.tex}

%\chapter[Web]
%{Definisi\\ Web}
%\input{section/1web.tex}

%\chapter[Backend]
%{Definisi\\ Backend}
%\input{section/1backend.tex}


%\chapter[Frontend]
%{Frontend}
%\input{section/1Frontend.tex}

<<<<<<< HEAD
\chapter[Method Delete]
{Delete}
\input{section/3flaskdelete.tex}

\chapter[Pengertian Web Service]
{pengertianwebservice}
\input{section/1pengertianwebservice.tex}
=======
>>>>>>> 05ae2201de81220de64aa49b84920e6bc847f17d

%\chapter[Frontend]
%{Frontend}
%\input{section/1frontend.tex}

%\chapter[Pengertian Web Service]
%{pengertianwebservice}
%\input{section/1pengertianwebservice.tex}

%\chapter[Port]
%{Port}
%\input{section/1Port.tex}


%\chapter[Aplikasi Web Service]
%{Aplikasi Web Service}
%\input{section/1aplikasiwebservice.tex}

%\chapter[Protokol]
%{Protokol}
%\input{section/1Protokol_kelompok3.tex}

%\chapter[Arsitektur Client Server]
%{Arsitektur Client Server}
%\input{section/2arsitekturclientserver.tex}


%\chapter[Common Gateway Interface]
%{Common Gateway Interface}
%\input{section/1CGIWSGIUWSGI.tex}

%\chapter[Common Gateway Interface]
%{Common Gateway Interface}
%\input{section/2CGIWSGIUWSGI.tex}


%\chapter{Instalasi PIP dan Contoh Penggunaan}
%{Instalasi PIP dan Contoh Penggunaan}
%\input{section/1instalasiPIPdancontohpenggunaan.tex}

%\chapter[Penggunaan Aplikasi Testing WebService]
%{Common Gateway Interface}
%\input{section/1penggunaanaplikasitestingwebservice.tex}

%\chapter[Pemanggilan Modul]
%{Common Gateway Interface}
%\input{section/2pemanggilanmodul.tex}

%\chapter[Instalasi PIP]
%{Common Gateway Interface}
%\input{section/1instalasiPIPdancontohpenggunaan.tex}

%\chapter[Variabel]
%{Variabel}
%\input{section/2Variabel.tex}

%\chapter[Penjelasan JSON dan YAML]
%{Penjelasan JSON dan YAML}
%\input{section/5PenjelasanJSONdanYAML.tex}

%\chapter[Fungsi Python]
%{Fungsi Python}
%\input{section/2fungipython.tex}

%\chapter[Hello World Python dan Identation]
%{Hello World dan Identation}
%\input{section/2HelloWorldPythondanIdentation.tex}

%\chapter[3postmandanswagger]
%{postmandanswagger}
%\input{section/3postmandanswagger}

%\chapter[Instalasi Flask]
%{Instalasi Flask}
%\input{section/3InstalasiFlask.tex}


%\chapter[Definisi Dekorator, Contoh Kode dan Fungsi]
%{Definisi Dekorator, Contoh Kode dan Fungsi}
%\input{section/3Dekorator.tex}

%\chapter[5RespondatauErorHandlingFlask]
%{5RespondatauErorHandlingFlask}
%\input{section/5RespondatauErorHandlingFlask.tex}

%\chapter[Postman dan Swagger]
%{Postman dan Swagger}
%\input{section/3postmandanswagger.tex}


%\chapter[Hello World di Flask]
%{Hello World di Flask}
%\input{section/3HelloWorldDiFlask.tex}


%\chapter[GET]
%{Parameter GET}
%\input{section/3GET.tex}


%\chapter[Json dan YAML]
%{JSON dan YAML}
%\input{section/5PenjelasanJSONdanYAML.tex}

%\chapter[Flask GET]
%{Flask GET}
%\input{section/3FlaskGet.tex}



%\chapter[Json dan YAML]
%{JSON dan YAML}
%\input{section/5PenjelasanJSONdanYAML.tex}


%\chapter[Upload File di Flask]
%{Upload File di Flask}
%\input{section/5CaraUploadFilediFlask.tex}



%\chapter[Membuat Aplikasi di Flask dan Swagger]
%{Membuat Aplikasi di Flask dan Swagger}
%\input{section/5CaraMembuatAplikasiDenganFlaskGer.tex}


%\chapter[Penggunaan Template di Flask]
%{Penggunaan Template di Flask}
%\input{section/5PenggunaanTemplatediFlask.tex}

%\chapter[Penggunaan Template di Flask]
%{Penggunaan Template di Flask}
%\input{section/5PenggunaanTemplatediFlask.tex}


% contoh aplikasi web service
% web service
% protokol
% port

% HTTP
% URL
% POST
% GET



\bibliographystyle{IEEEtran}.
%\bibliography{references}.

\printindex

\end{document}
