...
Variable merupakan suatu nama yang memiliki nilai data dimana nilai tersebut dapat berubah pada saat program dieksekusi atau 
suatu nilai tertentu di dalam proses program. Pada setiap bahasa pemograman, pemberian nilai kedalam variable (assignment) 
mempunyai bentuk penulisan yang berbeda dan nilai dari suatu variable bisa diubah-ubah sesuai kebutuhan. Tapi nama dari suatu 
varaiable dapat ditentukan sendiri oleh pemograman dengan aturan.
...
konstanta serta tidak menggunakan tanda dollar di depan nama konstantaya.
...
\subsection{Operator dalam PHP}
Sebuah operator dalam suatu Bahasa pemograman khususnya PHP , merupakan suatu symbol yang dapat digunakan atau dapat mempengaruhi nilai 
dari satu atau beberapa variable. Dalam bahaasa pemograman PHP mengenal berbagai macam jenis operator, yatu berupa operator penugasan, 
aritmatika, logika, perbandingan dan operator unray. Operator tersebung memiliki fungsinya masing-masing di dalam suatu variabel di 
satu Bahasa pemogramannya.
...
\item Tidak boleh ada spasi. Sebagai ganti spasi dapat menggunakan karakter underscore. Misalnya nama_dosen.
\item Tidak boleh menggunakan karakter-karakter khusus yang digunakan untuk operator dan lain-lain.
\end{enumerate}

\section{Deklarasi Variabel dan Variabel Dinamis}
Variabel di PHP bisa digunakan meskipun belum dideklarasikan. Penamaan variabel harus diawali dengan tanda dollar dan diikuti oleh nama ringkas. Nama variabel tidak boleh diawali dengan angka, tetapi bisa berisi angka dan karakter underscore. 
...
$$a = ‘World’;
\end{verbatim}
Tanda double dollar adalah sintaks yang digunakan untuk menuliskan suatu variabel-variabel. Variabel yang juga disebut variabel dinamis ini bisa dipanggil melalui dua cara.

\section{Penggunaan Variabel}
Variabel digunakan secara luas dalam penulisan program karena variabel akan meringkas proses penulisan dan membuat script (baris-baris perintah) mudah dimengerti, mudah dioperasikan, diubah, atau diperbaharui. 
Secara sederhana, variabel adalah sebuah perwakilan dari suatu nilai. Nilai yang kita wakilkan pada variabel akan disimpan dan dapat dipanggil kembali dengan menyebutkan nama variabel tersebut. 
...
\subsection{Menguji regresi yaitu dapat kita lakukan  dengan variabel moderating menggunakana MRA}
...
dengan X2 terhadap Y. 
 ...
 \subsection{Variabel intervening}
dalam sebuah penelitian merupakan kinerja keuangan sebuah perusahaan .kinerja keuangan disini maksunya suatu alasan yang dilakukan 
untuk dapat melihat sudah sejauh mana sebuah perusahhan tersebut telah melakukan sebuah kegiatan operasional dengan menggunakan aturan 
aturan pelaksanaan keuangan secara baik maupun secara benar(Fahmi,2012:2). Dalam penelitian kali ini indicator apa saja yang kita 
gunakan untuk mengukur tingkat ukuran kinerja keuangan ini yaitjnya adlaah Return on Aset(ROA). Yaitu merupakan sebuah ukuran 
efektifitas sebuah perusahhan pada saat menghasilkan keuntungan yaitu dengan cara memanfaatkan asset perusahaannya.
