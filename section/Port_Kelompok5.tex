...
\hspace{1cm}
Sesuai dengan fungsi sebuah Port yaitu media penghubung untuk melewatkan data masuk atau keluar pada jaringan komunikasi. Sebagai contoh jenis-jenis port yang digunakan pada beberapa protokol jaringan :\\
\begin{enumerate}
\item
 (Hypertext Transfer Protocol) adalah protocol yang
dipergunakan untuk mentransfer dokumen dalam World Wide Web (WWW). Menggunakan nama port 80 dan 443.
\item SSH adalah port jaringan komputer yang bersifat logic.Menggunakan nama port 22.
\item IMAP (Internet Message Access Protocol) adalah protokol standar untuk mengakses/mengambil email dari server. Menggunakan nama port 143.
\end{enumerate}
\end{document}
