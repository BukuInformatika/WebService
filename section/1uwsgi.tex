...
\item konfigurasi ke dalam regular expressions
\item Sekarang Anda dapat mengkonfigurasi Penangan Statis yang menunjuk ke subdirektori aplikasi
instalasi web2py Anda:\\
\verb  - Ingat untuk menyimpan konfigurasi, dan kembali atau mulai ulang Cherokee dari antarmuka administratif; maka Anda siap untuk memulai server uWSGI\\
\verb  - Ubah ke ID pengguna yang benar yang digunakan untuk menginstal web2py;\\
\verb  - Sadarilah bahwa menggunakan
root tidak disarankan.\\
\verb  - Pergilah ke direktori root dari instalasi web2py, di mana Anda menyimpan konfigurasi file uwsgi.xml.
\item Jalankan uWSGI dengan opsi -d <logfile>, sehingga berjalan di latar belakang:
\\ \$ su - <web2py user>
\\ \$ cd <web2py root>
...
\section{Contoh - Contoh uWSGI}
	Flask micro-framework merupakan pustaka Python yang dapat membantu untuk pembuatan Web. uWSGI merupakan implementasi protokol WSGI, uWSGI dapat menangani transmisi data antara kode Python . Perbedaan diantara uWSGI dan Flask adalah bahwa uWSGI berfungsi sebagai aplikasi namun tidak hanya aplikasi sekarang UWSGI dapat dijalankan dimana saja sedangkan Flask berfungsi untuk membantu pembuatan web \cite{mulerolinked}.\\
	uWSGI mendukung berbagai protokol dan cara untuk mengambil file  konfigurasi seperti stdin dan http. contohnya
\verb |\# uwsgi [option] [option 2] .. -w [wsgi.py with application callable]|\\
